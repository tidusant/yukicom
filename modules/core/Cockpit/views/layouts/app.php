...
<head>
    ...
    <!--duy.ha: fix routing to favicon.ico-->
    ...
    <!--duy.ha: include notify service to header-->
    {{ $app->assets(["assets:vendor/notifyservice.js"], $app['cockpit/version']) }}
    {{ $app->assets(["assets:vendor/notifyservice.css"], $app['cockpit/version']) }}
...
<body>
    ...
    <!-- duy.ha custom top menu -->
    <nav class="uk-navbar app-top-navbar" style="background-color:#444;position: fixed;width: 100%;top: 0;">

        <div class="app-wrapper">

            <ul class="uk-navbar-nav">
                <li class="uk-parent" data-uk-dropdown="{mode:'click'}">
                    <a href="@route('/dashboard')"  style="color:#fff;"><i class="uk-icon-bars"></i><strong class="uk-hidden-small"> &nbsp;{{ $app['app.name'] }}</strong></a>
           ...
           <!--  <div class="uk-navbar-content uk-hidden-small">
            ...
            <!-- duy.ha enhance menu top -->
                <ul class="uk-navbar-nav app-top-navbar-links">
                    @foreach($app("admin")->menu('top') as $item)
                    <li class="{{ (isset($item["active"]) && $item["active"]) ? 'uk-active':(isset($item["class"])?$item["class"]:'') }}" {{ isset($item["attr"])?$item["attr"]:'' }}>
                        <a style="color:#fff;" href="{{ $item["url"] }}" title="{{ $item["title"] }}" data-uk-tooltip>{{ $item["label"] }}</a>
    ...
    <div class="app-main" style="margin-top: 60px;padding-top: 20px;">
        <div class="app-wrapper">
            {{ $content_for_layout }}
        </div>
    </div>
