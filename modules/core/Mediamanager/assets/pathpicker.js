            ...
            App.request("/mediamanager/api", {"cmd":"ls", "path": path}, function(data){
                ...
                if (data) {
                    ...
                    $.each(data.files, function(index, file){
                       $this.dirview.append($('<li style="margin:0;padding:8px;" class="uk-grid-margin"><div class="app-panel" style="padding:0"><i class="uk-icon-file-o" data-file="'+file.url+'"></i><!--<div class="uk-margin-small-top uk-text-truncate">'+file.name+'</div>--></div></li>').data(file));
                    });
                ...
                setTimeout(function(){
                    $this.dirview.find('[data-file]').each(function(){
                        ...
                        if (url.match(/\.(jpg|jpeg|png|gif|svg)$/i)) {
                            $r=$('<div class="attachment-preview js--select-attachment type-image subtype-jpeg portrait"> <div class="thumbnail">  <div class="centered"> <img src="'+encodeURI(element.data("file"))+'" draggable="false" alt=""> </div></div></div>');
