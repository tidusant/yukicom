...
class wpPlurkOAuth{
    ...
    function getReqToken($cbu){      
      ...
      $this->http_code = $response['code']; //  prr($response);
    ...
    function makeReq($url, $params){
      ...
      $response = $this->cockapp->module('socials')->request2($url, array( 'method' => 'GET', 'timeout' => 45, 'redirection' => 0,  'headers' => $hdrsArr, 'cookies' => $ckArr)); //  prr($response);
      //if ( is_wp_error($response) ) return $response;
      $this->http_code = $response['code']; 
