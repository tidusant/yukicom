...
class wpScoopITOAuth{
    ...
    function getReqToken($cbu){      
      ...
      $response = $this->cockapp->module('socials')->request2($url, array( 'method' => 'GET', 'timeout' => 45, 'redirection' => 0,  'headers' => $hdrsArr, 'cookies' => $ckArr));  
    ...
    function makeReq($url, $params='', $type='GET'){
      ...
      if ( $type=='GET') {  $url .= '?'.$argsStr;  $hdrsArr = $this->makeHTTPHeaders($url);  $ckArr = ''; // prr($url);
          $response = $this->cockapp->module('socials')->request2($url, array( 'method' => 'GET', 'timeout' => 45, 'redirection' => 0,  'headers' => $hdrsArr)); //prr($response);
          ...
          //print_r($argsStr);exit;
          $response = $this->cockapp->module('socials')->request2($url, array( 'timeout' => 45, 'redirection' => 0, 'params'=>$argsStr,'method'=>$type,  'headers' => $hdrsArr)); //prr($argsStr); prr($argsT);   prr($response);
          //print_r($response);exit;
      }
      //print_r($response);exit;
      //if ( is_wp_error($response) ) return $response;
      //$this->http_code = $response['response']['code']; 
      $body = $response['body']; 
      $body = $this->cockapp->module('socials')->maybe_unserialize($body);     
      if (is_array($body)) {        
        return $body; 
      }
      else{
    ...
    private function sign($url, &$parameters) { 
        $string = implode("&", array( 'POST', rawurlencode($url), rawurlencode($this->joinParameters($parameters))));            
    ...
    public function flUploadPhoto ($imgFile, $parameters) { 
      ...
      $requestParams = ($parameters == NULL ? array() : $parameters);    $phid = '';    
        $oauthParams = array (            
            'oauth_nonce' => $this->genRndString(),
            'oauth_timestamp' => time(),            
            'oauth_token' => $this->access_token,
        ...
        //$xml = $this->cockapp->module('socials')->request2($url, $option); 
        //print_r($xml);exit;
        if (stripos($xml, '</photoid>')!==false && stripos($xml, 'stat="ok"')!==false) $phid = $this->cockapp->module('socials')->CutFromTo($xml, '<photoid>','</photoid>');
