(function($){
    ...
    angular.module('cockpit.fields').factory('Contentfields', function Contentfields() {
        ...
        var fields = {
            ...
            'select': {
                ...
                template: function(model, settings) {
                    ...
                    for (var i=0;i < options.length;i++) {
                        ...
                        //duy.ha : edit to use key as option's value, default use value as option's value
                        if(settings.useVal!=undefined && settings.useVal && typeof(options[i])=="object")
                            output.push('<option value="'+options[i].key+'">'+options[i].val+'</option>');                        
                        else
            ...
            'boolean': {
                label: 'Boolean',
                //duy.ha edit, custom label
                template: function(model, options) {
                    var label="",labelleft="",labelright="";                    
                    if(options.label!=undefined){
                        label="&nbsp;<span>"+options.label+"</span>&nbsp;";
                        if(options.labelRight!=undefined)
                            labelright=label;
                        else
                            labelleft=label;
                    }
                    ...
                    return '<div>'+labelleft+'<input name="'+name+'" type="checkbox" ng-model="'+model+'">'+labelright+'</div>';
                }
            },
